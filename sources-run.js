...
let modelsToRun = cli.models.length ? cli.models : [config.provider === 'openai' ? config.openaiModel : config.geminiModel];
...
// sources.csv: id,title,url,type,content  (RFC CSV, supports quotes/newlines)
const csvRaw = await fs.readFile("sources.csv","utf-8");
const recs = csvParse(csvRaw, { columns: true, skip_empty_lines: true });
...
async function summarize(provider, model, profileCfg, url, title, text){
  ...
  if (provider === 'openai') {
    const response = await openai.chat.completions.create({ model, messages: [{role:'system',content:systemPrompt},{role:'user',content:userPrompt}], temperature: 0.5 });
...
if (allSummaries.length > 0) {
  ...
  const rowsToAppend = allSummaries.map((s, idx) => buildCsvRow([
    existingLines + idx + 1, s.processed, s.source, s.url, s.title, s.contentLength, s.tokensUsed, s.provider, s.profile, s.model, s.cost.toFixed(6), s.summary, s.koreanSummary, s.key, s.articleId, s.profileConfigHash,
  ...
  console.log(`\n📊 Appended ${allSummaries.length} summaries to ${aggregatorPath}`);
}
